...
class irc_client(threading.Thread):
    ...
    def update_chat(self, message):
        ...
        for i in range(len(words)):
            if len(words[i]) < 30:
                if (len(words[i]) + len(new_line)) < 30:
                    new_line += words[i]+' '
                else:
    ...
    def get_channels(self):
        ...
        messages = abema.fetch_slots()
        for message in messages['slots']:
            channels_list.append(message)
...
class GUI(xbmcgui.WindowXMLDialog):
                    ...
                    if self.channels_list:
                        for i in self.channels_list:
                            ...
                            if channel == self.VideoTitle:
                                ...
                                self.item = ""
                                self.channel = '#%s' %self.VideoTitle
                                self.join_channel()
                                break
        else:
    ...
    def join_channel(self):
        #import web_pdb; web_pdb.set_trace()
        #color = xbmc.getInfoLabel('Skin.String(ChatMenuColorString)')
        xbmc.executebuiltin("Skin.ToggleSetting(ChatIsLoading)")
        if self.window.getProperty('inChannel') != 'True':
            ...
            if not self.item:
                if self.channel:
                    cid = addon.getSetting('channel_id')
                else:
                    return
            if not cid:
                for channel in self.channels_list:
                    if channel['title'] == self.item:
                        cid = channel['id']
                        addon.setSetting('channel_name', self.item)
                        addon.setSetting('channel_id', cid)
                        addon.setSetting('video_title', self.VideoTitle)
                        break
            if cid :
                self.client.join(cid)
